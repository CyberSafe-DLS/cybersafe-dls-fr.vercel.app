...
document.getElementById('nav-border').style.height = width; 
...
function debounce(func, wait) {
  ...
  function initSearch() {
    ...
    $searchInput.addEventListener("keyup", debounce(function() {
      ...
      if (results.length === 0) {
        $searchResultsHeader.innerText = `Aucun resultat pour «${term}»`;
        return;
      }
  
      currentTerm = term;
        $searchResultsHeader.innerText = `${results.length} résultat trouvé pour «${term}»:`;
